...
const SidebarUpdateService = ({ open, toggleSidebar, Data, hideSidebar }) => {
  ...
  const Brand = useSelector((state) => state.updateBrand);
  const { loading } = Brand;
  const updateSuccess = Brand?.Brand?.success;
  const updateError = Brand?.Brand?.error;
  ...
  const onSubmit = (data) => {
    ...
    dispatch(updateBrandAction(Data?._id, OBJ));
  };
